...
#define __TAG__ "ARSTREAMING_TCPReader_TB"
...
typedef struct {
    ...
    uint8_t *buffer;
    uint32_t index;
} ARSTREAMING_TCPReader_t;

typedef struct {
    uint32_t header;
    uint32_t size;
    uint32_t num;
} ARSTREAMING_TCPReader_FrameHeader_t;
 ...
 */
ARSTREAMING_TCPReader_t *ARSTREAMING_TCPReader_Create (int port, const char *ip);

/**
 * Send a frame through an ARSTREAMING_TCPReader_t
 */
int ARSTREAMING_TCPReader_ReadFrame (ARSTREAMING_TCPReader_t *reader, uint8_t *frame, uint32_t capacity);

/**
 * Delete an ARSTREAMING_TCPReader_t
 */
void ARSTREAMING_TCPReader_Delete (ARSTREAMING_TCPReader_t **reader);
...
ARSTREAMING_TCPReader_t *ARSTREAMING_TCPReader_Create (int port, const char *ip)
{
    ARSTREAMING_TCPReader_t *reader = malloc (sizeof (ARSTREAMING_TCPReader_t));
    if (reader == NULL)
    {
        return NULL;
    }
...
int ARSTREAMING_TCPReader_ReadFrame (ARSTREAMING_TCPReader_t *reader, uint8_t *frame, uint32_t capacity)
{
    ...
    int magicFound = 0;
    int magicIndex = 0;
    ARSTREAMING_TCPReader_FrameHeader_t header;
    while (magicFound == 0)
    {
...
int ARSTREAMING_TCPReader_Main (int argc, char *argv[])
{
    ...
    ARSTREAMING_TCPReader_t *reader = ARSTREAMING_TCPReader_Create (DATA_PORT, ip);
    ...
    while (stillRunning)
    {
        int size = ARSTREAMING_TCPReader_ReadFrame (reader, frameBuffer, FRAME_MAX_SIZE);
        if (size > 0)
        {
    ...
    ARSTREAMING_TCPReader_Delete (&reader);
