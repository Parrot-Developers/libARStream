 ...
 * @brief Implementation file for the Drone2-like streaming test
 * @date 07/10/2013
...
#define __TAG__ "ARSTREAMING_TCPSender_TB"
...
typedef struct {
    int socket;
    int csocket;
} ARSTREAMING_TCPSender_t;

typedef struct {
    uint32_t header;
    uint32_t size;
    uint32_t num;
} ARSTREAMING_TCPSender_FrameHeader_t;
 ...
 * This function generates dummy frames periodically, and sends them through the ARSTREAMING_TCPSender
 * @param params ARSTREAMING_TCPSender_t instance
 ...
 */
ARSTREAMING_TCPSender_t *ARSTREAMING_TCPSender_Create (int port);

/**
 * Send a frame through an ARSTREAMING_TCPSender_t
 */
void ARSTREAMING_TCPSender_SendFrame (ARSTREAMING_TCPSender_t *sender, ARSTREAMING_TCPSender_FrameHeader_t *header, uint8_t *frame);

/**
 * Delete an ARSTREAMING_TCPSender_t
 */
void ARSTREAMING_TCPSender_Delete (ARSTREAMING_TCPSender_t **sender);
...
{
    uint8_t *buffer;
    ARSTREAMING_TCPSender_FrameHeader_t header = { HEADER_MAGIC, 0, 0 };
    ARSTREAMING_TCPSender_t *sender = (ARSTREAMING_TCPSender_t *)param;
    srand (time (NULL));
    ...
    while (stillRunning)
    {
        ...
        ARSTREAMING_TCPSender_SendFrame (sender, &header, buffer);
...
ARSTREAMING_TCPSender_t *ARSTREAMING_TCPSender_Create (int port)
{
    ARSTREAMING_TCPSender_t *sender = malloc (sizeof (ARSTREAMING_TCPSender_t));
    if (sender == NULL)
    {
        return NULL;
    }
...
void ARSTREAMING_TCPSender_SendFrame (ARSTREAMING_TCPSender_t *sender, ARSTREAMING_TCPSender_FrameHeader_t *header, uint8_t *frame)
{
    ...
    ARSAL_Socket_Send (sender->csocket, header, sizeof (ARSTREAMING_TCPSender_FrameHeader_t), 0);
...
int ARSTREAMING_TCPSender_Main (int argc, char *argv[])
{
    ...
    ARSTREAMING_TCPSender_t *sender = ARSTREAMING_TCPSender_Create (DATA_PORT);
    ...
    ARSTREAMING_TCPSender_Delete (&sender);
