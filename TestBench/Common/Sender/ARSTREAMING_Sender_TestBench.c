/**
 * @file ARSTREAMING_Sender_TestBench.c
...
#define __TAG__ "ARSTREAMING_Sender_TB"
...
static ARSTREAMING_Sender_t *g_Sender;
...
float ARSTREAMING_Sender_PercentOk = 0.0;
static int nbSent = 0;
static int nbOk = 0;
 ...
 */
void ARSTREAMING_SenderTb_printUsage ();

/**
 * @brief Initializes the multi buffers of the testbench
 */
void ARSTREAMING_SenderTb_initMultiBuffers ();

/**
 * @see ARSTREAMING_Sender.h
 */
void ARSTREAMING_SenderTb_FrameUpdateCallback (eARSTREAMING_SENDER_STATUS status, uint8_t *framePointer, uint32_t frameSize, void *custom);

/**
 * @brief Gets a free buffer pointer
 * @param[in] buffer the buffer to mark as free
 */
void ARSTREAMING_SenderTb_SetBufferFree (uint8_t *buffer);
 ...
 */
uint8_t* ARSTREAMING_SenderTb_GetNextFreeBuffer (uint32_t *retSize);

/**
 * @brief Fake encoder thread function
 * This function generates dummy frames periodically, and sends them through the ARSTREAMING_Sender_t
 ...
 */
void* fakeEncoderThread (void *ARSTREAMING_Sender_t_Param);
 ...
 */
int ARSTREAMING_SenderTb_StartStreamingTest (ARNETWORK_Manager_t *manager);
...
void ARSTREAMING_SenderTb_FrameUpdateCallback (eARSTREAMING_SENDER_STATUS status, uint8_t *framePointer, uint32_t frameSize, void *custom)
{
    ...
    case ARSTREAMING_SENDER_STATUS_FRAME_SENT:
        ARSTREAMING_SenderTb_SetBufferFree (framePointer);
        ...
        nbSent++;
        nbOk++;
        ARSTREAMING_Sender_PercentOk = (100.f * nbOk) / (1.f * nbSent);
        break;
...
void* fakeEncoderThread (void *ARSTREAMING_Sender_t_Param)
{
    ...
    while (stillRunning)
    {
        ...
        nextFrameAddr = ARSTREAMING_SenderTb_GetNextFreeBuffer (&frameCapacity);
        if (nextFrameAddr != NULL)
        {
            if (frameCapacity >= frameSize)
            {
                eARSTREAMING_ERROR res;
                int nbPrevious;
                int flush = ((cnt % I_FRAME_EVERY_N) == 1) ? 1 : 0;
                memset (nextFrameAddr, cnt, frameSize);
                res = ARSTREAMING_Sender_SendNewFrame (sender, nextFrameAddr, frameSize, flush, &nbPrevious);
                switch (res)
                {
                case ARSTREAMING_OK:
...
int ARSTREAMING_SenderTb_StartStreamingTest (ARNETWORK_Manager_t *manager)
{
    ...
    ARSTREAMING_SenderTb_initMultiBuffers ();
    g_Sender = ARSTREAMING_Sender_New (manager, DATA_BUFFER_ID, ACK_BUFFER_ID, ARSTREAMING_SenderTb_FrameUpdateCallback, NB_BUFFERS, NULL, &err);
    if (g_Sender == NULL)
    {
    ...
    pthread_create (&streamingsend, NULL, ARSTREAMING_Sender_RunDataThread, g_Sender);
    pthread_create (&streamingread, NULL, ARSTREAMING_Sender_RunAckThread, g_Sender);
    ...
    ARSTREAMING_Sender_StopSender (g_Sender);

    pthread_join (streamingread, NULL);
    pthread_join (streamingsend, NULL);

    ARSTREAMING_Sender_Delete (&g_Sender);
...
int ARSTREAMING_Sender_TestBenchMain (int argc, char *argv[])
{
    ...
    if (3 <=  argc)
    {
        ARSTREAMING_SenderTb_printUsage ();
        return 1;
    }
    ...
    retVal = ARSTREAMING_SenderTb_StartStreamingTest (g_Manager);
...
float ARSTREAMING_SenderTb_GetEfficiency ()
{
    ...
    return ARSTREAMING_Sender_GetEstimatedEfficiency (g_Sender);
}
