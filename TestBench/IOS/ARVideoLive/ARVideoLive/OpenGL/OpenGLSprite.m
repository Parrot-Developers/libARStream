...
- (id)initWithFrame:(CGRect)frame withScaling:(ARDroneScaling)_scaling withProgram:(GLuint)programId withData:(ARDroneOpenGLTexture *)data withDelegate:(id)_delegate
{
	if ((self = [super init]))
	{
		// ScreenSize
        ...
        memset(default_image, 5, texture->texture_size.width * texture->texture_size.height * texture->bytesPerPixel);
        texture->dataSize = texture->texture_size.width * texture->texture_size.height * texture->bytesPerPixel;
...
- (void)drawSelf
{
   // opengl_context_t context;
    ...
    if(new_resolution)
    {            
        GLuint texScaleUniform = glGetUniformLocation(program, "u_texscale");
