...
- (BOOL) prepareRender;
- (void) setGLTexture:(AROpenGLTexture *)gltexture;
@end

@interface GLRenderer_RGB () <GLRenderer>
...
- (void) setGLTexture:(AROpenGLTexture *)gltexture
{
    ...
    glTexImage2D(GL_TEXTURE_2D,
                 0,
                 GL_RGBA,
                 gltexture->texture_size.width,
                 gltexture->texture_size.height,
                 0,
                 GL_RGBA,
                 GL_UNSIGNED_BYTE,
                 gltexture->data);
...
@interface GLRenderer_YUV () <GLRenderer>
@property (nonatomic, assign) GLint *uniformSamplers;
@property (nonatomic, assign) GLuint *textures;
@end
...
- (id)init
{
    ...
    if(self)
    {
        _uniformSamplers = malloc(sizeof(GLint) * 3);
        _textures = malloc(sizeof(GLuint) * 3);
    }
...
- (void) setGLTexture:(AROpenGLTexture *)gltexture
{
    ...
    unsigned char *y = gltexture->data;
    unsigned char *u = y + (int)(gltexture->texture_size.width * gltexture->texture_size.height);
    unsigned char *v = u + (int)(gltexture->texture_size.width * gltexture->texture_size.height / 4);
    const UInt8 *pixels[3] = { y, u, v };
    const NSUInteger widths[3]  = { gltexture->texture_size.width, gltexture->texture_size.width / 2, gltexture->texture_size.width / 2 };
    const NSUInteger heights[3] = { gltexture->texture_size.height, gltexture->texture_size.height / 2, gltexture->texture_size.height / 2 };
...
- (id) initWithFrame:(CGRect)frame rendererClass:(Class)rendererClass
{
    ...
    if (self) {
        _renderer = [[rendererClass alloc] init];
        NSLog(@"OK use %@ GL renderer", NSStringFromClass(rendererClass));
...
- (void)render:(AROpenGLTexture *)texture
{
    ...
    if (!_frameBufferReady) {
        NSLog(@"Framebuffer is not ready. No rendering will be performed until it is ready.");
        return;
    }
    ...
    if(texture != NULL && texture->data != NULL)
        [_renderer setGLTexture:texture];
