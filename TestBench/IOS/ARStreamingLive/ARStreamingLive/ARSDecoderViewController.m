...
@property (nonatomic, strong) ARSStreamingReader *reader;
...
@synthesize ip;
@synthesize streamingHeight;
@synthesize streamingWidth;
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self)
    {
        _reader = [[ARSStreamingReader alloc] init];
...
#else
    CGSize resolution = { streamingWidth, streamingHeight };
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [_reader start:ip];
    [_decoder initializeWithWidth:streamingWidth andHeight:streamingHeight];
    ...
    while (! [_decodeThread isCancelled])
    {
        ...
        if (frame)
        {
            ...
            if (_decodedFrame)
            {
                ...
                _texture.image_size.height = streamingHeight;
                _texture.image_size.width  = streamingWidth;
                _texture.texture_size.height = streamingHeight;
                _texture.texture_size.width  = streamingWidth;
