...
#import "ARSTREAMING_Reader_TestBench.h"
#import "ARSTREAMING_Sender_TestBench.h"
#import "ARSTREAMING_TCPReader.h"
#import "ARSTREAMING_TCPSender.h"

#define kARSTREAMINGNumberOfPoints 50
#define kARSTREAMINGDeltaTTimerSec 0.1
...
- (void)viewDidLoad
{
    ...
    plotSpace.xRange                = [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat(0.0) length:CPTDecimalFromFloat(kARSTREAMINGNumberOfPoints * 1.f)];
    ...
    x.majorIntervalLength         = CPTDecimalFromInt(kARSTREAMINGNumberOfPoints);
    ...
    latencyGraphData = [[NSMutableArray alloc] initWithCapacity:kARSTREAMINGNumberOfPoints];
    ...
    lossFramesData = [[NSMutableArray alloc] initWithCapacity:kARSTREAMINGNumberOfPoints];
    ...
    deltaTData = [[NSMutableArray alloc] initWithCapacity:kARSTREAMINGNumberOfPoints];
    ...
    efficiencyData = [[NSMutableArray alloc] initWithCapacity:kARSTREAMINGNumberOfPoints];
...
- (void)resetDataArrays
{
    ...
    for (int i = 0; i < kARSTREAMINGNumberOfPoints; i++)
    {
...
- (void) startTimer
{
    _refreshTimer = [NSTimer timerWithTimeInterval:kARSTREAMINGDeltaTTimerSec target:self selector:@selector(updateStatus) userInfo:nil repeats:YES];
...
- (void) updateStatus
{
    ...
    else
    {
        switch (_mode)
        {
            case ARS_Sender:
            {
                [percentOk setText:[NSString stringWithFormat:@"%d%%", ARSTREAMING_SenderTb_GetEstimatedLoss()]];
                int estLat = ARSTREAMING_SenderTb_GetLatency();
                if (estLat < 0)
                {
                ...
                [self refreshGraphs];
                [logger log:[NSString stringWithFormat:@"%4d; %5.2f; %3d; %4d; %5.3f", estLat, ARSTREAMING_Sender_PercentOk, missed, dt, eff]];
            }
                break;
            case ARS_Reader:
            {
                [percentOk setText:[NSString stringWithFormat:@"%d%%", ARSTREAMING_ReaderTb_GetEstimatedLoss()]];
                int estLat = ARSTREAMING_ReaderTb_GetLatency();
                if (estLat < 0)
                {
                    [latency setText:@"Not connected"];
                    if ([latencyGraphData count] > 0)
                ...
                [self refreshGraphs];
                [logger log:[NSString stringWithFormat:@"%4d; %5.2f; %3d; %4d; %5.3f", estLat, ARSTREAMING_Reader_PercentOk, missed, dt, eff]];
            }
                break;
            case ARS_TCP_Sender:
...
- (void)runSender
{
    ...
    char *name = "ARStreaming_TestBench_iOS";
    ...
    ARSTREAMING_Sender_TestBenchMain(2, params);
    [logger close];
    logger = nil;
}
...
- (void)runReader
{
    ...
    logger = [[ARSLogger alloc] init];
    [logger log:@"Latency (ms); PercentOK (%%); Missed frames; Mean time between frame (ms); Efficiency"];
    ARSTREAMING_Reader_TestBenchMain(2, params);
    [logger close];
    logger = nil;
}
...
- (void)runTcpSender
{
    ...
    ARSTREAMING_TCPSender_Main(1, params);
}
...
- (void)runTcpReader
{
    ...
    ARSTREAMING_TCPReader_Main(2, params);
    [logger close];
    logger = nil;
}
...
- (void)stop:(id)sender
{
    ARSTREAMING_Sender_TestBenchStop();
    ARSTREAMING_Reader_TestBenchStop();
    ARSTREAMING_TCPReader_Stop();
    ARSTREAMING_TCPSender_Stop();
    [self resetDataArrays];
    [self stopTimer];
