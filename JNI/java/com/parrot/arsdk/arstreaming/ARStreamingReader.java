...
/**
 * Wrapper class for the ARSTREAMING_Reader C object.<br>
 * <br>
 * To create an ARStreamingReader, the application must provide a suitable
 * <code>ARStreamingReaderListener</code> to handle the events.<br>
 * <br>
 * The two ARStreamingReader Runnables must be run in independant threads<br>
 */
public class ARStreamingReader
{
    private static final String TAG = ARStreamingReader.class.getSimpleName ();
     ...
     * Event listener
     */
    private ARStreamingReaderListener eventListener;
    ...
    /**
     * Constructor for ARStreamingReader object<br>
     * Create a new instance of an ARStreamingReader for a given ARNetworkManager,
     ...
     */
    public ARStreamingReader (ARNetworkManager netManager, int dataBufferId, int ackBufferId, ARNativeData initialFrameBuffer, ARStreamingReaderListener theEventListener) {
        ...
        if (this.cReader != 0) {
            ...
            this.dataRunnable = new Runnable () {
                    public void run () {
                        nativeRunDataThread (ARStreamingReader.this.cReader);
                    }
                };
            this.ackRunnable = new Runnable () {
                    public void run () {
                        nativeRunAckThread (ARStreamingReader.this.cReader);
                    }
                };
        } else {
            this.valid = false;
    ...
    /**
     * Stops the internal threads of the ARStreamingReader.<br>
     * Calling this function allow the ARStreamingReader Runnables to end
     */
    public void stop () {
        nativeStop (cReader);
    }
    ...
    /**
     * Deletes the ARStreamingReader.<br>
     * This function should only be called after <code>stop()</code><br>
     * <br>
     * Warning: If this function returns <code>false</code>, then the ARStreamingReader was not disposed !
     * @return <code>true</code> if the Runnables are not running.<br><code>false</code> if the ARStreamingReader could not be disposed now.
     */
    ...
    private long[] callbackWrapper (int icause, long ndPointer, int ndSize, boolean isFlush, int nbSkip) {
        ARSTREAMING_READER_CAUSE_ENUM cause = ARSTREAMING_READER_CAUSE_ENUM.getFromValue (icause);
        if (cause == null) {
     ...
     * This function created a C-Managed ARSTREAMING_Reader object
     ...
     * @return C-Pointer to the ARSTREAMING_Reader object (or null if any error occured)
     */
     ...
     * @param cReader C-Pointer to the ARSTREAMING_Reader C object
     */
