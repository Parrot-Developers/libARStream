...
/**
 * Wrapper class for the ARSTREAMING_Sender C object.<br>
 * <br>
 * To create an ARStreamingSender, the application must provide a suitable
 * <code>ARStreamingSenderListener</code> to handle the events.<br>
 * <br>
 * The two ARStreamingSender Runnables must be run in independant threads<br>
 */
public class ARStreamingSender
{
    private static final String TAG = ARStreamingSender.class.getSimpleName ();
     ...
     * Event listener
     */
    private ARStreamingSenderListener eventListener;
     ...
     * @return A new buffer, configured to be an ARStreaming data buffer
     */
     ...
     * @param bufferId Id of the new buffer
     * @return A new buffer, configured to be an ARStreaming ack buffer
     */
    ...
    /**
     * Constructor for ARStreamingSender object<br>
     * Create a new instance of an ARStreamingSender for a given ARNetworkManager,
     ...
     */
    public ARStreamingSender (ARNetworkManager netManager, int dataBufferId, int ackBufferId, ARStreamingSenderListener theEventListener, int frameBufferSize) {
        ...
        if (this.cSender != 0) {
            ...
            this.dataRunnable = new Runnable () {
                    public void run () {
                        nativeRunDataThread (ARStreamingSender.this.cSender);
                    }
                };
            this.ackRunnable = new Runnable () {
                    public void run () {
                        nativeRunAckThread (ARStreamingSender.this.cSender);
                    }
                };
        } else {
            this.valid = false;
    ...
    /**
     * Stops the internal threads of the ARStreamingSender.<br>
     * Calling this function allow the ARStreamingSender Runnables to end
     */
    public void stop () {
        nativeStop (cSender);
    }
    ...
    /**
     * Deletes the ARStreamingSender.<br>
     * This function should only be called after <code>stop()</code><br>
     * <br>
     * Warning: If this function returns <code>false</code>, then the ARStreamingSender was not disposed !
     * @return <code>true</code> if the Runnables are not running.<br><code>false</code> if the ARStreamingSender could not be disposed now.
     */
    ...
    /**
     * Sends a new frame with the ARStreamingSender.<br>
     ...
     * @param flush If active, the ARStreamingSender will cancel any remaining prevous frame, and start sending this one immediately
     */
    ARSTREAMING_ERROR_ENUM SendNewFrame (ARNativeData frame, boolean flush) {
        return ARSTREAMING_ERROR_ENUM.ARSTREAMING_OK;
    }
    ...
    private void callbackWrapper (int istatus, long ndPointer, int ndSize) {
        ARSTREAMING_SENDER_STATUS_ENUM status = ARSTREAMING_SENDER_STATUS_ENUM.getFromValue (istatus);
        if (status == null) {
            return;
        }
     ...
     * This function created a C-Managed ARSTREAMING_Sender object
     ...
     * @return C-Pointer to the ARSTREAMING_Sender object (or null if any error occured)
     */
     ...
     * @param cSender C-Pointer to the ARSTREAMING_Sender C object
     */
